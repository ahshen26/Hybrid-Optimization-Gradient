...
Simulation = function (dataset, 
  ...
  dataset$hazardRatioCov = predict(coxModel, newdata = dataset, type = "risk", reference = "zero")
  # Get baseine hazard ratio
  BaselineHazardRatio = flexsurvreg(Surv(STIME, STATUS) ~ 1, data = dataset, dist = "exp")
  BaselineHazardRatio = BaselineHazardRatio$res[1]

  SurvivalFunction = function(hazardRatio, t) {
    return(exp(-hazardRatio*t))
  }
  ...
  for (i in 1:nsim) {
    ...
    enrolledPatients = sample(nrow(dataset), N, replace = FALSE)
    ...
    #exposure
    monthsTRT = sum(data.match[data.match$TRT == 1, "STIME"])
    monthsCONTROL = sum(data.match[data.match$TRT == 0 & data.match$ENROLL == 1, "STIME"])
    ...
    res = rbind(res, c(hr_trt_control, hr_control_RWD, hr_borrow, Z1, Z2, Z3, B1, B2, sigma_Z1, sigma_Z2, sigma_Z3, var_B1, var_B2, covZ1Z2, covB1B2, w, n_events_trt, n_events_control, n_events_RWD, monthsTRT, monthsCONTROL))
  }
  res = as.data.frame(res)
  colnames(res) = c("hr_trt_control", "hr_control_RWD", "hr_borrow", "Z1", "Z2", "Z3", "B1", "B2", "sigma_Z1", "sigma_Z2", "sigma_Z3", "var_B1", "var_B2", "covZ1Z2", "covB1B2", "w", "n_events_trt", "n_events_control", "n_events_RWD", "TRTExposure", "ControlExposure")
  return(res)
}
