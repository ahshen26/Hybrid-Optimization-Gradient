...
def fun_Power(input, weight, N, q, effectSize, bias, x1_var, x2_var, x3_var, cov_Z1Z2, alpha, alpha_EQ, calibration):
    ...
    if dimension == 3:
        ...
        beta_array = np.zeros((n_col, n_row))
    ...
    for m in range(n_col):
        for n in range(n_row):
            ...
            if theta < 0: theta = 0
            beta = norm.cdf(theta, x2_mean, np.sqrt(x2_var)) - norm.cdf(-theta, x2_mean, np.sqrt(x2_var))
            ...
            if dimension == 3:
                ...
                cost_array[m, n] = (1 - power[len(power) // 2] - weight * N_t + (100*(max(typeIerror)-0.07) if max(typeIerror) > 0.07 else 0) + (100*((power_reference - 0.03) - min(power)) if min(power) < (power_reference - 0.03) else 0))*100
                ...
                cost_array[m] = (1 - power[len(power) // 2] - weight * N_t + (100*(max(typeIerror)-0.07) if max(typeIerror) > 0.07 else 0) + (100*((power_reference - 0.03) - min(power)) if min(power) < (power_reference - 0.03) else 0))*100
                ...
                cost_array = (1 - power[len(power) // 2] - weight * N_t + (100*(max(typeIerror)-0.07) if max(typeIerror) > 0.07 else 0) + (100*((power_reference - 0.03) - min(power)) if min(power) < (power_reference - 0.03) else 0))*100
