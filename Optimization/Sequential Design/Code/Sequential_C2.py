...
def fun_Power2(input):
    ...
    def root_function(cutoffValue):
        return fun_cutoffValueFinder(cutoffValue, 0, 0, 0, W2_var, x2_var_stage1, W1_var, corr_w2x2, theta)[0] - alpha
    ...
    for j in np.arange(-0.6, 0.61, 0.05):
        ...
        typeIerror_ul = multivariate_normal.cdf([np.inf, -theta], mean=np.array([0, j]), cov=np.array([[W2_var, cov_w2x2], [cov_w2x2, x2_var_stage1]])) - multivariate_normal.cdf([cutoffValue*np.sqrt(W2_var), -theta], mean=np.array([0, j]), cov=np.array([[W2_var, cov_w2x2], [cov_w2x2, x2_var_stage1]]))
        # Non-borrow: upperright
        typeIerror_ur = 1 - multivariate_normal.cdf([np.inf, theta], mean=np.array([0, j]), cov=np.array([[W2_var, cov_w2x2], [cov_w2x2, x2_var_stage1]])) - multivariate_normal.cdf([cutoffValue*np.sqrt(W2_var), np.inf], mean=np.array([0, j]), cov=np.array([[W2_var, cov_w2x2], [cov_w2x2, x2_var_stage1]])) + multivariate_normal.cdf([cutoffValue*np.sqrt(W2_var), theta], mean=np.array([0, j]), cov=np.array([[W2_var, cov_w2x2], [cov_w2x2, x2_var_stage1]]))
        # Non-borrow: buttomright
        typeIerror_br = multivariate_normal.cdf([-cutoffValue*np.sqrt(W2_var), np.inf], mean=np.array([0, j]), cov=np.array([[W2_var, cov_w2x2], [cov_w2x2, x2_var_stage1]])) - multivariate_normal.cdf([-cutoffValue*np.sqrt(W2_var), theta], mean=np.array([0, j]), cov=np.array([[W2_var, cov_w2x2], [cov_w2x2, x2_var_stage1]]))
        # Non-borrow: buttomleft
        typeIerror_bl = multivariate_normal.cdf([-cutoffValue*np.sqrt(W2_var), -theta], mean=np.array([0, j]), cov=np.array([[W2_var, cov_w2x2], [cov_w2x2, x2_var_stage1]]))
        # Borrow: upper
        typeIerror_u = multivariate_normal.cdf([np.inf, theta], mean=np.array([-v1*w_stage1*j - (1-v1)*w_stage2*j, j]), cov=np.array([[W1_var, 0], [0, x2_var_stage1]])) - multivariate_normal.cdf([cutoffValue*np.sqrt(W1_var), theta], mean=np.array([-v1*w_stage1*j - (1-v1)*w_stage2*j, j]), cov=np.array([[W1_var, 0], [0, x2_var_stage1]])) - multivariate_normal.cdf([np.inf, -theta], mean=np.array([-v1*w_stage1*j - (1-v1)*w_stage2*j, j]), cov=np.array([[W1_var, 0], [0, x2_var_stage1]])) + multivariate_normal.cdf([cutoffValue*np.sqrt(W1_var), -theta], mean=np.array([-v1*w_stage1*j - (1-v1)*w_stage2*j, j]), cov=np.array([[W1_var, 0], [0, x2_var_stage1]]))
        # Borrow: buttom
        typeIerror_b = multivariate_normal.cdf([-cutoffValue*np.sqrt(W1_var), theta], mean=np.array([-v1*w_stage1*j - (1-v1)*w_stage2*j, j]), cov=np.array([[W1_var, 0], [0, x2_var_stage1]])) - multivariate_normal.cdf([-cutoffValue*np.sqrt(W1_var), -theta], mean=np.array([-v1*w_stage1*j - (1-v1)*w_stage2*j, j]), cov=np.array([[W1_var, 0], [0, x2_var_stage1]]))
        ...
        # Non-borrow: upperleft
        Power_ul = multivariate_normal.cdf([np.inf, -theta], mean=np.array([W2_mean, j]), cov=np.array([[W2_var, cov_w2x2], [cov_w2x2, x2_var_stage1]])) - multivariate_normal.cdf([cutoffValue*np.sqrt(W2_var), -theta], mean=np.array([W2_mean, j]), cov=np.array([[W2_var, cov_w2x2], [cov_w2x2, x2_var_stage1]]))
        # Non-borrow: upperright
        ...
        # Non-borrow: buttomright
        Power_br = multivariate_normal.cdf([-cutoffValue*np.sqrt(W2_var), np.inf], mean=np.array([W2_mean, j]), cov=np.array([[W2_var, cov_w2x2], [cov_w2x2, x2_var_stage1]])) - multivariate_normal.cdf([-cutoffValue*np.sqrt(W2_var), theta], mean=np.array([W2_mean, j]), cov=np.array([[W2_var, cov_w2x2], [cov_w2x2, x2_var_stage1]]))
        # Non-borrow: buttomleft
        Power_bl = multivariate_normal.cdf([-cutoffValue*np.sqrt(W2_var), -theta], mean=np.array([W2_mean, j]), cov=np.array([[W2_var, cov_w2x2], [cov_w2x2, x2_var_stage1]]))
        # Borrow: upper
        Power_u = multivariate_normal.cdf([np.inf, theta], mean=np.array([W1_mean, j]), cov=np.array([[W1_var, 0], [0, x2_var_stage1]])) - multivariate_normal.cdf([cutoffValue*np.sqrt(W1_var), theta], mean=np.array([W1_mean, j]), cov=np.array([[W1_var, 0], [0, x2_var_stage1]])) - multivariate_normal.cdf([np.inf, -theta], mean=np.array([W1_mean, j]), cov=np.array([[W1_var, 0], [0, x2_var_stage1]])) + multivariate_normal.cdf([cutoffValue*np.sqrt(W1_var), -theta], mean=np.array([W1_mean, j]), cov=np.array([[W1_var, 0], [0, x2_var_stage1]]))
        # Borrow: buttom
        Power_b = multivariate_normal.cdf([-cutoffValue*np.sqrt(W1_var), theta], mean=np.array([W1_mean, j]), cov=np.array([[W1_var, 0], [0, x2_var_stage1]])) - multivariate_normal.cdf([-cutoffValue*np.sqrt(W1_var), -theta], mean=np.array([W1_mean, j]), cov=np.array([[W1_var, 0], [0, x2_var_stage1]]))
