...
for i in range(1,5):
  ...
  optimalDesign.append(fitted_results.x)")
    ...
    borrow3 = abs (Z2/sigma_Z2) <= theta3
    borrow4 = abs (Z2/sigma_Z2) <= theta4
    ...
    typeIerror_nonborrowing = alpha_p*(1-v)
    
    #. If we split too much to non borrowing cases:
    if (typeIerror_nonborrowing > max_value_nonborrowing) {
      cutoffValue_nonborrowing = 0
    ...
    res = rbind(res, data.frame(hr_trt_control = hr_trt_control,
                                ...
                                cutoffValue_borrowing = cutoffValue_borrowing,
                                cutoffValue_nonborrowing = cutoffValue_nonborrowing,
  ...
  type1Error_s <- (sum(ifelse(res$borrow3, res$Z3 < -res$cutoffValue_borrowing, res$Z1 < -res$cutoffValue_nonborrowing)) + sum(ifelse(res$borrow3, res$Z3 > res$cutoffValue_borrowing, res$Z1 > res$cutoffValue_nonborrowing)))/nrow(res)
  type1Error_new = (sum(ifelse(res$borrow4, res$Z3 < -res$cutoffValue_new, res$Z1 < qnorm (alpha_p/2)*sqrt(res$var_Z1))) + sum(ifelse(res$borrow4, res$Z3 > res$cutoffValue_new, res$Z1 > qnorm (1-alpha_p/2)*sqrt(res$var_Z1))))/nrow(res)
