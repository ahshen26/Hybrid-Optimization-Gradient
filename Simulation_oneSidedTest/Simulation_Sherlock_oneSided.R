...
inference = function (dataset, alpha_p, delta, alphaEQ) {
  ...
  type1Error_b = (sum (res$estimator/res$estimator.sd > qnorm(1 - alpha_p))) / nrow(res)
  type1Error_b2 = (sum (res$estimator2/res$estimator.sd2 > qnorm(1 - alpha_p))) / nrow(res)
  type1Error_b3 = (sum (res$estimator3/res$estimator.sd3 > qnorm(1 - alpha_p))) / nrow(res)
  type1Error_new = (sum(ifelse(res$borrow, res$Z3 > res$cutoffValue_new, res$Z1 > qnorm (1-alpha_p)*sqrt(res$var_Z1))))/nrow(res)
  return (list(res = res,
